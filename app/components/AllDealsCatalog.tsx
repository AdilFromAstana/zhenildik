...
interface AllDealsCatalogProps {
    ...
    onOpenFilters: () => void;
    onNavigate: (page: Page, deal?: Deal) => void;
}

const AllDealsCatalog: React.FC<AllDealsCatalogProps> = ({
    onDetailClick,
    filters,
    setFilters,
    onOpenFilters,
                    ...
                    <div className="flex space-x-3 overflow-x-scroll pb-2 scrollbar-hide">
                        {['Все', ...categories.map((c) => c.name)].map((name) => {
                            ...
                            const isActive =
                                ...
                                (name === 'Все' && filters.category === '');

                            return (
                                <button
                                    key={name}
                                    onClick={() =>
                                        handleFilterChange('category', name === 'Все' ? '' : name)
                                    }
                                    className={`flex items-center justify-center flex-shrink-0 px-4 py-2 text-sm rounded-full transition duration-150 border ${isActive
                            ...
                            {activeFilters.length > 0 && (
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {activeFilters.map(([key, value]) => (
                                        <div
                                            key={key}
                                            className="flex items-center bg-blue-100 text-blue-700 text-xs font-medium px-3 py-1 rounded-full"
                                        >
                                            {key === 'city'
                                                ? 'Город'
                                                : key === 'discountType'
                                            ...
                                            <button
                                                ...
                                                className="ml-1.5 p-0.5 hover:text-blue-900"
                                            >
                                                <X className="w-3 h-3" />
                                            </button>
                                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {displayedDeals.length > 0 ? (
                            ...
                            <p className="col-span-full text-center text-gray-500 py-10">
                                По вашим фильтрам акций не найдено. Попробуйте изменить критерии
                                поиска.
                            </p>
                        )}
                    </div>
                    ...
                    {!hasMore && allFilteredDeals.length > 0 && (
                        <div className="col-span-full text-center py-6">
                            <p className="text-gray-500 font-medium">
                                Вы достигли конца списка акций ({allFilteredDeals.length} шт.).
                            </p>
                        </div>
                    )}
                </section>
