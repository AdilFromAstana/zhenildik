...
const ASTANA_BOUNDS: [[number, number], [number, number]] = [
    [71.25, 51.00], // юго-запад
    [71.70, 51.30], // северо-восток
];

const ASTANA_CENTER: [number, number] = [51.1282, 71.4304];

const AddressSearchMap: React.FC<AddressSearchMapProps> = ({ onAddressSelect }) => {
    ...
    const handleSearch = async (q: string) => {
        ...
        try {
            const res = await ymapsRef.current.geocode(q.trim(), {
            ...
            const withDistances = arr.map((obj: any) => {
                ...
                const city =
                    meta?.Address?.Components?.find(
                        (c: any) => c.kind === "locality"
                    )?.name || "";
                return { obj, coords, distance, city };
            });

            // фильтруем только адреса, где город — Астана
            const filtered = withDistances.filter((item) =>
                /астан/i.test(item.city)
            );
    ...
    return (
        <div className="w-full h-[75vh] relative">
            ...
            <div id="map" className="h-full rounded-lg overflow-hidden border border-gray-200" />
        </div>
    );
};
